...
โปรแกรมนี้ได้รับการพัฒนาและผ่านการทดสอบตาม Test Plan อย่างเข้มข้นจำนวน 6 กรณี (รวม Edge Cases และโบนัส) ดังนี้:

## 🚀 ฟีเจอร์หลัก (Features & Test Cases)

โปรแกรมนี้ได้รับการพัฒนาและผ่านการทดสอบตาม Test Plan จำนวน 5 กรณี ครอบคลุมการทำงานหลักของระบบ ดังนี้:

1. **Partial Loading (จำกัดช่วงข้อมูล):** สามารถระบุ `Start Row` และ `End Row` เพื่อโหลดและแสดงข้อมูลเฉพาะช่วงที่ต้องการจากไฟล์ CSV ได้อย่างถูกต้อง
2. **Invalid Range Validation (ตรวจสอบช่วงข้อมูลผิดพลาด):** มีระบบตรวจสอบเงื่อนไขการกรอกข้อมูล หาก `Start Row` มากกว่า `End Row` โปรแกรมจะแสดงข้อความแจ้งเตือน (MessageBox) และยกเลิกการทำงานเพื่อป้องกันข้อผิดพลาด
3. **Data Filtering (ระบบกรองข้อมูล):** สามารถระบุ Keyword ในช่อง Filter เพื่อค้นหาและแสดงเฉพาะข้อมูลที่ตรงกับคำค้นหา เช่น `exe` โดยรองรับการค้นหาแบบไม่สนใจตัวพิมพ์เล็ก-ใหญ่ (Case-Insensitive)
4. **Combined Condition (Range + Filter):** รองรับการใช้งานร่วมกันระหว่างการกำหนดช่วงข้อมูล (Start Row - End Row) และการกรองข้อมูล (Filter) เพื่อให้แสดงผลเฉพาะข้อมูลที่ตรงตามทั้งสองเงื่อนไข
5. **Exception Handling (จัดการข้อผิดพลาด):** ใช้โครงสร้าง `try-catch` เพื่อดักจับข้อผิดพลาดระหว่างการอ่านไฟล์ เช่น ไม่พบไฟล์ หรือ Path ไม่ถูกต้อง พร้อมแสดงข้อความแจ้งเตือนแก่ผู้ใช้งาน
